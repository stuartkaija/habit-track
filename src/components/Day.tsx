...
export default function Day({ date, createdToday, day, week, completionStatus, handleUpdateHabit }: any) {
  ...
  if (!date) {
    ...
    <div className="group relative w-max">
      <div
        className={`h-3 w-3 border  rounded-sm cursor-pointer ${createdToday ? 'border-orange-500 shadow-sm shadow-orange-300' : ''} ${isToday(date) ? 'border-blue-500 shadow-sm shadow-blue-300' : ''} ${completionStatus ? 'bg-green-300' : 'bg-gray-100'}`}
